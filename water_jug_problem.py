...
def water_jugs(jug_a, jug_b, target):
    ...
    while queue:
        #each iteration will pop the oldest state form the queue. a=3 gallon jug, b=5 gallon jug
        (a,b), path = queue.popleft()

        #check if we reached the target, then exit if we are done
        if a == target or b == target:
            if a == target:
                final_state = (a, 0) #Emptying other jug to match the exactly goal amount
                final_action = "Emptying other jug to match the exactly goal amount"
            else:
                final_state = (0, b) #empty the other jug to match the exactly goal amount
                final_action = "Emptying other jug to match the exactly goal amount"
